            ...
            <c:forEach var="productNew" items="${getproductNewTop8List}">
                <div class="column">
                    ...
                    <c:when test="${fn:length(title) > 12}">
                        <h2>${fn:substring(title, 0, 9)}...</h2>
                        </c:when>
                        <c:otherwise>
                        <h2>${title}</h2>
                            </c:otherwise>
                            </c:choose>
                        <p>${productNew.product_price}원</p>
                </div>
            </c:forEach>
        </div>
    </div>
    <div class="container_right">
        ...
        <table>
            <c:forEach var="popularTop" items="${getproductpopularTop8List}" varStatus="status" >
                <tr>
                    <td><img src="${popularTop.product_img}"/></td>
                    <td>
                        <table>
                            <tr>
                                <td class="book_rank">${status.count}위</td>
                            </tr>
                            <tr>
                                <c:set var="pwriter" value="${popularTop.product_writer}"/>
                                <c:choose>
                                    <c:when test="${fn:length(pwriter) > 10}">
                                <td class="book_rank_writer">${fn:substring(pwriter, 0, 9)}...</td>
                                    </c:when>
                                    <c:otherwise>
                                <td class="book_rank_writer">${pwriter}</td>
                                    </c:otherwise>
                                </c:choose>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td class="book_rank_sales">${popularTop.product_sale}개 판매!</td>
                            </tr>
                        </table>
                    </td>
                </tr>
