function mailCheck(){
    if(document.frm.emails.value == "1"){
...
function infocheck(){
    ...
    if (document.frm.name.length == 0){
        alert("이름을 입력해주세요.");
        document.frm.name.focus();
        return false;
    }
    if (document.frm.email1.length == 0){
        alert("이메일을 입력해주세요.");
        document.frm.email1.focus();
        return false;
    }
    if (document.frm.email2.value.match(emailExp) != null){
    ...
    if (document.frm.thrnum.value.match(phonExp) != null){
        alert("전화번호가 틀렸습니다.");
        document.frm.secnum.focus();
        return false;
    }
    if (document.frm.sample6_postcode.length == 0){
        alert("주소를 입력해주세요.");
